...
public class PotionEffectManager {
    ...
    // 药水效果配置
    private final Map<String, Integer> normalEffects = new HashMap<>();  // 普通效果
    private final Map<String, Integer> specialEffects = new HashMap<>();  // 特殊效果
    private final Map<String, Integer> negativeEffects = new HashMap<>();  // 负面效果
    private final Map<String, String> effectChineseNames = new HashMap<>();  // 中文名称映射
    private int maxEffectsPerItem;  // 每个物品最大效果数量
    private boolean potionEffectsEnabled;  // 是否启用药水效果功能
    ...
    public PotionEffectData getRandomNormalEffect(List<PotionEffectData> currentEffects) {
        ...
        String selected = availableEffects.get(random.nextInt(availableEffects.size()));
    ...
    public PotionEffectData getRandomNegativeEffect() {
        ...
        String selected = effects.get(random.nextInt(effects.size()));
        int level = random.nextInt(negativeEffects.get(selected)) + 1;
